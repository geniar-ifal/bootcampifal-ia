...
function calculaMinimax(nodo) {
  ...
  if (nodo.jogador == "o") nodo.minimax = max;
  // se a pr�xima jogada � da CPU, retorna valor max
...
function jogaHumano(elemento) {
  ...
  if (estado[i][j] != undefined) {
    showMessageDanger(
      `Posição [${i.toString()}, ${j.toString()}] já selecionada espertinho(a). Selecione outra.`
    );
    return;
  } else {
  ...
  if (!ehTerminal(estado, 1)) {
    ...
    if (!raiz) {
      ...
      for (i = 0; i < atual.filhos.length; i++) {
        // procura nos filhos do estado atual, qual representa a situa��o ap�s a jogada do humano
...
function showFirstMessage() {
  $(".messages").append(`<div class="alert alert-dark" role="alert">
		Vamos jogar!
	</div>`);
}
...
function showModalEmpate() {
  $("#textoModalJogoDaVelha").text(
    "Empatamos, nada mal hein. Vamos jogar novamente."
  );
  showModalJogoDaVelha();
}

function showModalDerrota() {
  $("#textoModalJogoDaVelha").text(
    "Perdeu Playboy, eu ganhei. Vamos jogar novamente."
  );
  showModalJogoDaVelha();
}

function showModalJogoDaVelha() {
  $("#modalJogoDaVelha").modal({
    show: true,
  });
}
